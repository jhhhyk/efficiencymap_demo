...
초기 구현에서는 `loadLane` 응답의 `section[].station[]` 필드에서 정류장 좌표를 가져왔습니다. 그러나 실제 호출에서 이 필드가 **비어 있는 경우가 반복적으로 발생**해 승·하차 마커가 표시되지 않는 문제가 있었습니다.

### 해결

API 명세를 확인한 결과, `searchPubTransPathT` 응답의 `subPath[].passStopList.stations[]`에 정류장명과 함께 `x`·`y` 좌표가 포함되어 있으며, 이 필드는 **명세상 필수 항목**이었습니다. 이미 호출하고 있는 응답 안에 더 신뢰할 수 있는 데이터가 있었던 셈입니다.

따라서 데이터 출처를 다음과 같이 분리했습니다.
...
좌표를 소수점 5자리로 절삭해 키를 생성하므로, 미세한 좌표 차이로 인한 캐시 미스를 방지합니다.
